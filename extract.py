...
def fill_in(data,invoice_total):
    from openpyxl import load_workbook
    from openpyxl.styles import Alignment
    from openpyxl.styles import Font
    from datetime import datetime
    today_date=datetime.now().strftime("%d-%m-%Y")
    wb=load_workbook("DISPATCH-2.xlsx")
    count=21
    ws=wb.active
    ws["L14"]=today_date
    ...
    ws["T27"]=(data["total_amount"])
    ws["T27"].font=Font(bold="True")
    ws["T25"]=invoice_total*0.025
    ws["T25"].font=Font(bold="True")
    ws["T26"]=invoice_total*0.025
    ws["T26"].font=Font(bold="True")
    ws["T28"]=invoice_total*0.001
    ws["T28"].font=Font(bold="True")
    ws["T30"]=((data["total_amount"])-(invoice_total*0.001))
